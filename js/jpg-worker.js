...
self.onmessage = async (e) => {
    const { id, imgData, quality, qPercent } = e.data;
    ...
    function smoothChromaYCbCr(d, w, h, strength) {
        ...
        for(let i = 0, p = 0; i < d.data.length; i += 4, p++) {
            const { y, cb, cr } = rgbToYCbCr(d.data[i], d.data[i+1], d.data[i+2]);
            yArr[p] = y; cbArr[p] = cb; crArr[p] = cr;
        }

        const cbCopy = cbArr.slice(), crCopy = crArr.slice();
            ...
            for(let x = radius; x < w - radius; x++) {
                ...
                for(let dy = -radius; dy <= radius; dy++)
                    for(let dx = -radius; dx <= radius; dx++){
                        ...
                        sumCb += cbCopy[idx]; sumCr += crCopy[idx]; count++;
                    }
                const i = y*w + x;
                const isRed = crCopy[i] > 150 && cbCopy[i] < 120;
                const localStrength = isRed ? Math.min(0.6, strength*1.8) : strength;
